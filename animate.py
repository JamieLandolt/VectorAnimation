...
class VectorTest(VectorScene):
    ...
    def construct(self):
        ...
        for i, (freq, (x, y)) in enumerate(reversed(vector_info)):
            ...
            vector_pos = [x + vector_info[i - 1][1][0], y + vector_info[i - 1][1][1]] if i != 0 else [x, y]
        ...
        path = TracedPath(vecs[-1].get_end, dissipating_time=0.5, stroke_width=4, stroke_color=BLUE_A, min_distance_to_new_point=0.01)
        self.add(path)
